...
def goal_find(savedColor):
    ...
    yellow_lower = np.array([150, 150, 0], np.uint8)
    yellow_upper = np.array([240, 240, 30], np.uint8)
    ...
    try:
        while True:
            ...
            if (count > 50 and target == 0):
                #print((cv2.countNonZero(orange_mask) / orange_mask.size))
                if (((cv2.countNonZero(color_mask) / color_mask.size) < 0.002) or ((cv2.countNonZero(color_mask) / color_mask.size) > 0.5)):
                    motors = 5000
                    tango.setTarget(MOTORS,motors)
                    time.sleep(0.2)
                    motors = 6000
